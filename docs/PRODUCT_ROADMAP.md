...
- **Status:** Umgesetzt (2026-07-07). Produktentscheidung: bei fehlender/ungültiger/abgelaufener Lizenz zeigt der Start einen klaren Warndialog, die Anwendung bleibt aber nutzbar (kein Hard-Lock, da es keine Online-Reaktivierung gibt und ein kaputtes lokales Lizenzfile sonst den Datenzugriff komplett sperren würde). Das aktive-Nutzer-Limit der Lizenz wird dagegen hart durchgesetzt: Neuanlage oder Reaktivierung eines Nutzers über dem Limit hinaus wird mit klarer Fehlermeldung abgelehnt. Zuvor prüfte der produktive Start (`secure_app.create_app`) die Lizenz gar nicht; das ist jetzt behoben. Ein Klick auf den Lizenzstatus im Kopfbereich öffnet zusätzlich einen Dialog mit Lizenzdetails, Kaufhinweis (Vertriebskontakt per `DIENSTPLANER_SALES_CONTACT` konfigurierbar) und einem Textfeld, um eine neu erhaltene, signierte Lizenz sofort freizuschalten. Weiterhin keine Online-Aktivierung, kein Lizenzserver.
- **Ergänzung (2026-07-07):** Fehlt beim Start eine Lizenzdatei, läuft die Anwendung ab dem ersten Start 30 Tage als Testversion ohne Warndialog (Kopfbereich zeigt „Testversion" mit verbleibenden Tagen). Das Startdatum der Testphase wird neben dem Lizenzpfad gespeichert, überlebt also Neuinstallationen und ein Löschen der Lizenzdatei. Nach Ablauf der 30 Tage greift dieselbe Nicht-Hard-Lock-Logik wie bei fehlender Lizenz: Warndialog, aber weiter nutzbar.
